...
void initWebServer(){  
  ...
  #endif
  domain = mainConfigDoc["settings"]["mDNS"]|DEFAULT_MDNS;
...
void notifyClients() {
  if(ws.availableForWriteAll() && ws.count()){
    char json[512];
    sprintf(json, "{\"currentSWR\":%.2f,\"clients\":%d}", globalData.currentSWR, globalData.numClients);
...
void handleWebSocketMessage(void *arg, uint8_t *data, size_t len, AsyncWebSocketClient *client) {
  ...
  if(tempDoc.containsKey("settings")){
...
void onEvent(AsyncWebSocket *server, AsyncWebSocketClient *client, AwsEventType type, void *arg, uint8_t *data, size_t len) {
  switch (type) {
    case WS_EVT_CONNECT:
      ...
      globalData.numClients = ws.count();
      notifyClients();
      break;
